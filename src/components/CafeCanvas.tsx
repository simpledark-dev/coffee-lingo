...
import {
  getEffectiveTile,
  buildFurnishingSpriteMap,
  buildPositionToItemMap,
} from "../lib/furnishing";
import {
  TILESET_ENABLED,
  SPRITE_KEY_TO_TILE,
...
// Auto-map every T property name to its sprite key (char → "PROP_NAME").
// Only override where the sprite key differs from the T property name.
const TILE_ID_TO_SPRITE: Record<string, string> = {
  ...Object.fromEntries(Object.entries(T).map(([k, v]) => [v as string, k])),
  [T.FLOOR]:    "FLOOR_WOOD",
  [T.CHAIR_U]:  "CHAIR",
  [T.CHAIR_D]:  "CHAIR",
  [T.MENU]:     "MENU_BOARD",
...
function drawTilesetAsset(
  ctx: CanvasRenderingContext2D,
  tilesetMap: Map<string, TilesetRef>,
  asset: number | { index: number; w: number; h: number; anchor?: "top-left" | "bottom-left"; sheetSrc?: string },
  dx: number,
  dy: number,
  tilePx: number
) {
  if (typeof asset === "number") {
    // Fallback for hardcoded number entries in SPRITE_KEY_TO_TILE — use first loaded sheet
    const tileset = tilesetMap.values().next().value;
    if (!tileset) return;
    drawTilesetIndex(ctx, tileset, asset, dx, dy, tilePx, tilePx);
    return;
  }

  const tileset = asset.sheetSrc ? tilesetMap.get(asset.sheetSrc) : tilesetMap.values().next().value;
  if (!tileset) return;

  const w = Math.max(1, Math.floor(asset.w || 1));
  const h = Math.max(1, Math.floor(asset.h || 1));
  ...
  const sx = col * tileset.tileSize;
  const sy = row * tileset.tileSize;
  const sw = w * tileset.tileSize;
  const sh = h * tileset.tileSize;

  const anchor = asset.anchor ?? (h > 1 ? "bottom-left" : "top-left");
  const drawX = dx;
  const drawY = anchor === "bottom-left" ? dy - (h - 1) * tilePx : dy;

  ctx.imageSmoothingEnabled = false;
  ctx.drawImage(tileset.img, sx, sy, sw, sh, drawX, drawY, w * tilePx, h * tilePx);
}
...
export default function CafeCanvas({
  ...
  const tilesetMappingRef = useRef<Record<string, any>>({
    ...(SPRITE_KEY_TO_TILE as Record<string, any>),
  });

  function loadSheetIntoMap(src: string, tileSize: number, alive: () => boolean) {
    if (tilesetMapRef.current.has(src)) return;
    const img = new Image();
    img.crossOrigin = "anonymous";
    img.onload = () => {
      if (!alive()) return;
      const cols = Math.max(1, Math.floor((img.naturalWidth || 0) / tileSize));
      tilesetMapRef.current.set(src, { img, cols, tileSize });
    };
    img.src = src;
  }
  ...
  useEffect(() => {
    if (!TILESET_ENABLED) return;
    let _alive = true;
    const alive = () => _alive;
    (async () => {
      try {
        const res = await fetch("/tilesheet-assets.json", { cache: "no-store" });
        if (!res.ok) return;
        const json = await res.json();
        if (!_alive) return;
        const fromFile = buildSpriteKeyToTileFromExport(json);
        tilesetMappingRef.current = {
          ...(SPRITE_KEY_TO_TILE as Record<string, any>),
          ...fromFile,
        };
        // Load all sheets from the sheets array (new format)
        if (Array.isArray(json?.sheets)) {
          for (const s of json.sheets) {
            if (s?.src) loadSheetIntoMap(s.src, s.tileW || 16, alive);
          }
        }
        // Fallback: derive sheets from assets directly (old format or missing sheets array)
        if (Array.isArray(json?.assets)) {
          for (const a of json.assets) {
            if (a?.sheetSrc && !tilesetMapRef.current.has(a.sheetSrc)) {
              loadSheetIntoMap(a.sheetSrc, a.sheetTileSize || 16, alive);
            }
          }
        }
      } catch {
  ...
  useEffect(() => {
    ...
    const targetRoom =
      placingItem.room === "patio" ? 0 : placingItem.room === "reading" ? 1 : 2;
  ...
  const drawScene = useCallback(
    (
      ...
      const tileset = TILESET_ENABLED ? tilesetMapRef.current : null;
      const tileMap = tilesetMappingRef.current;

      const startCol = Math.max(0, Math.floor(camX / TILE_PX));
      const endCol = Math.min(
        GRID_COLS - 1,
        Math.floor((camX + viewW) / TILE_PX)
      );
      const startRow = Math.max(0, Math.floor(camY / TILE_PX));
      const endRow = Math.min(
        GRID_ROWS - 1,
        Math.floor((camY + viewH) / TILE_PX)
      );
      ...
      for (let row = startRow; row <= endRow; row++) {
        for (let col = startCol; col <= endCol; col++) {
          const tileId = tileOverrides?.size
            ? getEffectiveTile(row, col, tileOverrides)
            : CAFE_LAYOUT[row]?.[col] ?? T.EMPTY;
          ...
          const asset = tileMap[floorStyle];
          if (tileset?.size && asset !== undefined)
            drawTilesetAsset(ctx, tileset, asset as any, px, py, TILE_PX);
            else drawCached(ctx, cache, floor, px, py);
          }

          if (tileId === T.WALL) {
          const asset = tileMap["WALL"];
          if (tileset?.size && asset !== undefined)
            drawTilesetAsset(ctx, tileset, asset as any, px, py, TILE_PX);
            else drawCached(ctx, cache, WALL, px, py);
            continue;
          }
          ...
          if (tileId === T.FOUNTAIN) {
            ...
            drawCached(
              ctx,
              cache,
              rightIsFountain ? FOUNTAIN_L : FOUNTAIN_R,
              px,
              py
            );
            continue;
          ...
          const spriteName =
            furnishingSpriteMapRef.current.get(posKey) ??
            resolvedTileSprite[tileId];
          if (spriteName) {
          const asset = tileMap[spriteName];
          if (tileset?.size && asset !== undefined) {
            drawTilesetAsset(ctx, tileset, asset as any, px, py, TILE_PX);
            } else if (SPRITE_MAP[spriteName]) {
  ...
  useEffect(() => {
    ...
    function onTouchMove(e: TouchEvent) {
      ...
      if (gestureDirRef.current === "vertical") {
        ...
        if (sceneRef.current === "outside") {
          ...
          outsideCamYRef.current = Math.max(
            0,
            Math.min(maxY, outsideCamStartYRef.current + dy)
          );
        } else {
          const maxY = Math.max(0, WORLD_H - viewHRef.current);
          cameraYRef.current = Math.max(
            0,
            Math.min(maxY, cameraStartYRef.current + dy)
          );
    ...
    function onTouchEnd(e: TouchEvent) {
      if (isDraggingRef.current) {
        if (touchMovedRef.current < 10) {
          ...
          if (
            swipeDxRef.current > SWIPE_THRESHOLD &&
            currentRoomRef.current < 2
          ) {
            switchToRoom(currentRoomRef.current + 1);
          } else if (
            swipeDxRef.current < -SWIPE_THRESHOLD &&
            currentRoomRef.current > minRoom
          ) {
            switchToRoom(currentRoomRef.current - 1);
          }
          velocityYRef.current = 0;
    ...
    function handleTap(clientX: number, clientY: number) {
      ...
      if (
        Math.sqrt((worldX - doorWorldX) ** 2 + (worldY - doorWorldY) ** 2) < 50
      ) {
        ...
        outsideCamYRef.current = Math.max(
          0,
          cafeCenterY - viewHRef.current / 2
        );
        return;
      }
      ...
      if (!patioUnlockedRef.current) {
        ...
        if (
          Math.sqrt((worldX - lockX) ** 2 + (worldY - lockY) ** 2) <
          TILE_PX * 0.7
        ) {
          onPatioLockTap?.();
          return;
      ...
      const tappedCol = Math.floor(worldX / TILE_PX);
      const tappedRow = Math.floor(worldY / TILE_PX);
      const tappedItemId = posToItemMapRef.current.get(
        `${tappedRow},${tappedCol}`
      );
      if (tappedItemId) {
    ...
    function onWheel(e: WheelEvent) {
      ...
      if (sceneRef.current === "outside") {
        ...
        outsideCamYRef.current = Math.max(
          0,
          Math.min(maxY, outsideCamYRef.current + e.deltaY)
        );
      } else {
        const maxY = Math.max(0, WORLD_H - viewHRef.current);
        cameraYRef.current = Math.max(
          0,
          Math.min(maxY, cameraYRef.current + e.deltaY)
        );
      }
    }
  ...
  useEffect(() => {
    ...
    function drawInterior(viewW: number, viewH: number) {
      ...
      cameraYRef.current = Math.max(
        0,
        Math.min(WORLD_H - viewH, cameraYRef.current)
      );
      ...
      if (state) {
        const sorted = state.characters
          .filter((c) => c.location === "interior")
          .sort((a, b) => a.worldPos.y - b.worldPos.y);
        ...
        if (alertCheckCounter >= 30) {
          ...
          const hasAlert = state.characters.some((c) => {
            if (c.location !== "interior" || c.phase !== "exclamation")
              return false;
      ...
      if (!patioUnlockedRef.current) {
        for (const dr of [21, 22]) {
          ...
          if (
            wallPx > -TILE_PX &&
            wallPx < viewW + TILE_PX &&
            wallPy > -TILE_PX &&
            wallPy < viewH + TILE_PX
          ) {
      ...
      if (fUpgrades && fPlaced) {
        ...
        for (const [posKey, itemId] of posMap.entries()) {
          ...
          if (remaining === 0) {
            ...
            const progress =
              level.upgrading.durationMs > 0
                ? 1 - remaining / level.upgrading.durationMs
                : 0;
            ctx.save();
            ctx.beginPath();
    ...
    function drawOutside(viewW: number, viewH: number) {
      ...
      outsideCamYRef.current = Math.max(
        0,
        Math.min(maxY, outsideCamYRef.current)
      );
      ...
      const endCol = Math.min(
        OUTSIDE_COLS - 1,
        Math.floor((bgCamX + viewW) / TILE_PX)
      );
      const startRow = Math.max(0, Math.floor(camY / TILE_PX));
      const endRow = Math.min(
        OUTSIDE_ROWS - 1,
      ...
      const SHOP_TILE_IDS: Set<string> = new Set([
        T.SHOP_WALL,
        T.SHOP_WALL_LIGHT,
        T.SHOP_WINDOW,
        T.LOCKED_DOOR,
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        if (NEEDS_FLOOR.has(tileId)) {
          const asset = tileMap[floorStyle];
          if (tileset?.size && asset !== undefined)
            drawTilesetAsset(ctx, tileset, asset as any, px, py, pxPerTile);
          else drawCached(ctx, cache, floorSpriteRef.current, px, py);
        }
  ...
  return (
    <div ref={containerRef} style={styles.container}>
      ...
      {scene === "outside" && <div style={styles.roomLabel}>Outside</div>}
      {/* Bottom buttons */}
      {!hideBottomButtons && (
        <>
          <button
        ...
        <div style={styles.mapOverlay} onClick={() => setShowMap(false)}>
          <div style={styles.mapPopup} onClick={(e) => e.stopPropagation()}>
            <div style={styles.mapHeader}>
              <span>Full Map</span>
              <button style={styles.mapClose} onClick={() => setShowMap(false)}>
                ✕
              </button>
            </div>
